...
void pop_queue_to_run(Process *&list, priority_queue<Process, vector<Process>, compare> &hang_doi)
{
...
void print_Queue(queue<Process> hang_doi){
...
void SRTN(Process *list, int num_process, double total_usage_time){
    priority_queue<Process, vector<Process>, compare> hang_doi;
    queue<Process> thu_tu_chay;
    
    double step = 0.5;
    double temp_time_end = 0;
    int index = 0;
    for(double i = 0; i <= total_usage_time; i += step)
    {
        ...
        if(index != -1)
        {
            ...
            {
                Process temp = hang_doi.top();
                if(temp.timeRemaining < list[index].timeRemaining)
                {   
                    cout << "Tai thoi diem " << i << " tien trinh " << hang_doi.top().name << " bat dau chay!" << endl << endl;
                    // thu_tu_chay.push(hang_doi.top());
                    pop_queue_to_run(list, hang_doi);
...
int main(){
    ...
    SRTN(list, num_process, total_usage_time);
