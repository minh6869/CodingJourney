...
void add_sinh_vien_vao_cuoi_danh_sach(sinhvien *&p , int &n, int &cot1, int &cot2, int &cot3, int &cot4)
{   
    sinhvien tmp;
    cout << "Nhap thong tin cua sinh vien moi: " << endl << "Ma Sinh Vien: ";
    fflush(stdin);
    getline(cin,tmp.msv);
    if(tmp.msv.size() > cot1)
        cot1 = tmp.msv.size();
    
    cout << "Ho va Ten: ";
    getline(cin,tmp.hoten);
    if (tmp.hoten.size() > cot2)
        cot2 = tmp.hoten.size();
    
    cout << "Lop: ";
    getline(cin,tmp.lop);
    if(tmp.lop.size() > cot3)
        cot3 = tmp.lop.size();
...
int main()
{
    ...
    xuat(p,n,cot1,cot2,cot3,cot4);
    add_sinh_vien_vao_cuoi_danh_sach(p,n,cot1,cot2,cot3,cot4);
}