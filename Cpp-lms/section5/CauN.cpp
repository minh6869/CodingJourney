...
void xuat_danh_sach_sinh_vien_co_diem_max_cua_mot_lop(sinhvien *p, int n, int cot1, int cot2, int cot3, int cot4)
{
    string tmplop;
    int max = 0;
    int dem = 0;
    ...
    for (int i=0; i<n ; i++)
    {
        if (tmplop == p[i].lop && p[i].diem >= max)
        {
            max = p[i].diem;
            dem++;
        }
    ...
    if (!dem)
        cout << "Khong co du lieu sinh vien cua lop " << tmplop << "!" << endl;
    else
    {
        cout << "Danh sach sinh vien co diem cao nhat cua lop " << tmplop << " la:" << endl;
        for (int i=0 ; i<n ; i++)
        {
            if(tmplop == p[i].lop && p[i].diem == max)
               cout << left << setw(cot1+3) << p[i].msv << left << setw(cot2+3) << p[i].hoten << left << setw(cot3+3) << p[i].lop << left << setw(cot4+3) << p[i].diem << endl; 
...
int main()
{
    ...
    xuat_danh_sach_sinh_vien_co_diem_max_cua_mot_lop(p,n,cot1,cot2,cot3,cot4);  
    return 0;
}